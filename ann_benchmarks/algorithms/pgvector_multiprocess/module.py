...
class PGVector_MultiProccess(BaseANN):
    def __init__(self, metric, method_param):
        self._metric = metric
        self._m = method_param['M']
        self._ef_construction = method_param['efConstruction']
        self._ef_search = method_param['efSearch']
        self._cur = None
        self._psycopg_connect_kwargs = None
    ...
    def fit(self, X):
        ...
        conn = psycopg.connect(**self._psycopg_connect_kwargs)
    ...
    def parallel_query(self, n, proc_id):
        ...
        cursor.execute("SET hnsw.ef_search = %d" % self._ef_search)
        cursor.execute("SHOW hnsw.ef_search")
        print("ef_search:", cursor.fetchall())

        base = (self._query_size // self._parallel) * proc_id
        end = min(self._query_size, base + self._query_size // self._parallel)
        local_cost_list = [0] * (end - base)

        _s = time.time()
        for i in range(base, end):
            tmp = self._query_data[i]
    ...
    def batch_query(self, X, n) -> None:
        print("batch query topk = %d" % n)
        self._query_data = X.repeat(self._parallel, axis=0)
        self._query_size = X.shape[0] * self._parallel
        procs = []
        print("query using %d parallel" % self._parallel)
